...
from modules.model.Krea2Model import (
    PROMPT_MAX_LENGTH,
    PROMPT_TEMPLATE_CROP_START,
    PROMPT_TEMPLATE_NUM_SUFFIX_TOKENS,
...
@factory.register(BaseDataLoader, ModelType.KREA_2)
class Krea2BaseDataLoader(
    ...
    def _preparation_modules(self, config: TrainConfig, model: Krea2Model):
        ...
        tokenize_prompt = Tokenize(in_name='prompt', tokens_out_name='tokens', mask_out_name='tokens_mask', tokenizer=model.tokenizer, max_token_length=PROMPT_MAX_LENGTH,
                                   format_text=PROMPT_TEMPLATE_PREFIX, additional_format_text_tokens=PROMPT_TEMPLATE_CROP_START - PROMPT_TEMPLATE_NUM_SUFFIX_TOKENS,
                                   suffix_text=PROMPT_TEMPLATE_SUFFIX)
        encode_prompt = EncodeQwenText(tokens_name='tokens', tokens_attention_mask_in_name='tokens_mask', hidden_state_out_name='text_encoder_hidden_state', tokens_attention_mask_out_name='tokens_mask',
                                       text_encoder=model.text_encoder, hidden_state_output_index=list(TEXT_ENCODER_SELECT_LAYERS), autocast_contexts=[model.autocast_context], dtype=model.train_dtype.torch_dtype(),
                                       crop_start=PROMPT_TEMPLATE_CROP_START, cumsum_position_ids=True)
    ...
    def _debug_modules(self, config: TrainConfig, model: Krea2Model): #TODO clean up
    ...
    def _create_dataset(
        ...
        return DataLoaderText2ImageMixin._create_dataset(self,
            ...
            vae_frame_dim=True,  #...Krea 2 has a video-capable VAE (same class as Qwen-Image). convert images to video dimensions
        )
